...
const myPow = (a, b, myPrint) => {
    ...
    if (b < 0) {
        const result = 1 / myPow(a, -b, null);
        return myPrint(a, b, result);
    }
   
    const result = a * myPow(a, b - 1, null);
    if (myPrint == null) {
        return result
    }
    return myPrint(a, b, result);
...
console.log(myPow(3, 4, myPrint));  // Виведе '3^4=81'
console.log(myPow(2, 3, myPrint));   // Виведе '2^3=8'
console.log(myPow(2, 0, myPrint));   // Виведе '2^0=1'
console.log(myPow(2, -2, myPrint));  // Виведе '2^-2=0.25'
 ...
 */
const bind = (func, value) => (x) => func(value, x);
const myDouble = myMul.bind(this, 2)
...
// Аналогічним чином створюємо функцію myTriple(n), яка потроює параметр, що приймає, повертаючи результат.
 ...
 const myTriple = myMul.bind(this, 3)
